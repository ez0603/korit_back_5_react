...
export function useLoadListByPageNumber(page) {
    ...
    const loadBordList = useMemo(() => {
        const lsBoardList = localStorage.getItem("boardList");
        const loadBordList = !lsBoardList ? [] : JSON.parse(lsBoardList);
        return loadBordList;
    }, [page]);
    
    const boardList = loadBordList.filter((board, index) => index > (pageNumber * 10) - 11 && index < pageNumber * 10);

    const size = loadBordList.length;

    //5페이지마다 넘어가게 만들기
    const totalPageCount = Math.floor(size % 10 === 0 ? size / 10 : (size / 10) + 1); // 페이지 제일 마지막 번호
